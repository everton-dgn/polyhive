...
const polyhiveHome = await mkdtemp(join(tmpdir(), "polyhive-onboard-home-"));
const port = await getAvailablePort();

try {
  console.log("Test 1: `polyhive` runs blocking onboarding and prints pairing info");
  const onboard =
    await $`POLYHIVE_HOME=${polyhiveHome} POLYHIVE_LISTEN=127.0.0.1:${port} POLYHIVE_PAIRING_QR=0 npx polyhive`.nothrow();
  ...
  assert(
    onboard.stdout.includes("polyhive --help"),
    "onboard output should include --help shortcut",
  );
  assert(onboard.stdout.includes("polyhive ls"), "onboard output should include ls shortcut");
  assert(
    onboard.stdout.includes('polyhive run "your prompt"'),
    "onboard output should include run shortcut",
  );
  assert(
    onboard.stdout.includes("polyhive status"),
    "onboard output should include status shortcut",
  );
  assert(
    onboard.stdout.includes(join(polyhiveHome, "daemon.log")),
    "onboard output should include daemon log path",
  );

  const status =
    await $`POLYHIVE_HOME=${polyhiveHome} npx polyhive daemon status --home ${polyhiveHome}`.nothrow();
  ...
  const configRaw = await readFile(join(polyhiveHome, "config.json"), "utf-8");
  ...
  const daemonLog = await readFile(join(polyhiveHome, "daemon.log"), "utf-8");
  assert(
