...
describe("VoiceAssistantWebSocketServer notification payloads", () => {
  ...
  it("uses assistant preview text for in-app notifications with markdown removed", async () => {
  ...
  it("builds notification payload regardless of UI label presence", async () => {
    ...
    expect(readAttentionRequiredMessage(ws).notification.body).toBe("Done.");
  ...
  it("does not mark a client without heartbeat for notification", async () => {
    const { server } = createServer();
    const ws = connectClient(server, null);
  ...
  it("does not mark error attention for a client without heartbeat", async () => {
    const { server } = createServer();
    const ws = connectClient(server, null);
