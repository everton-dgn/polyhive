 ...
 *
 * Tests daemon commands with an isolated POLYHIVE_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalPolyHive(["daemon", ...args], { POLYHIVE_HOME: polyhiveHome });
}
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalPolyHive(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, polyhiveHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
