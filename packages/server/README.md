...
POLYHIVE_HOME=~/.polyhive        # Runtime state directory (agents/, etc.)
POLYHIVE_LISTEN=127.0.0.1:6767  # Listen address (host:port or /path/to/socket)
```

`POLYHIVE_HOME` defaults to `~/.polyhive` and isolates runtime artifacts like `agents/`. `POLYHIVE_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
POLYHIVE_HOME=~/.polyhive-blue POLYHIVE_LISTEN=127.0.0.1:7777 npm run dev
```
