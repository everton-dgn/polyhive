...
2. **Beta flow**: you want public test builds first, but you are not ready for the website, npm, or production release flows to move yet.
...
The `NPM Publish` workflow at `.github/workflows/npm-publish.yml` runs on every stable tag push and publishes the four public packages (`@evertondgn/polyhive-cli`, `@evertondgn/polyhive-server`, `@evertondgn/polyhive-relay`, `@evertondgn/polyhive-highlight`) with [npm provenance](https://docs.npmjs.com/generating-provenance-statements) attached.
...
- Betas publish desktop assets for testing, but they do not publish npm packages and do not trigger the production web release flow
...
- `desktop-vX.Y.Z` and `desktop-macos-vX.Y.Z` rebuild only the macOS desktop release
...
- **Lead with the outcome.** "Agents launch reliably from npm shims…" is better than "Agents launch reliably across common install shapes. Claude, Codex, and OpenCode now start correctly…".
...
Format: append `([#123](https://github.com/everton-dgn/polyhive/pull/123) by [@user](https://github.com/user))` at the end of each bullet. For changes spanning multiple PRs or contributors:

```markdown
- Voice mode now works on tablets with proper microphone permissions. ([#210](https://github.com/everton-dgn/polyhive/pull/210), [#215](https://github.com/everton-dgn/polyhive/pull/215) by [@alice](https://github.com/alice), [@bob](https://github.com/bob))
```

Rules:

- **Always link the PR number** as `[#N](https://github.com/everton-dgn/polyhive/pull/N)`.
