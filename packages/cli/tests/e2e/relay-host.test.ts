...
// e2e suites (packages/relay/src/e2e.test.ts, packages/server/.../relay-transport.e2e.test.ts).
...
async function probeRelayConnection(offerUrl: string): Promise<boolean> {
  ...
  const client = new DaemonClient({
    ...
    webSocketFactory: (target: string, opts?: { headers?: Record<string, string> }) =>
      new WebSocket(target, { headers: opts?.headers }) as unknown as WebSocketLike,
  });
  try {
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      timeout: 60_000,
      env: {
        POLYHIVE_RELAY_ENABLED: "true",
        POLYHIVE_RELAY_ENDPOINT: relayEndpoint,
        POLYHIVE_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });

    const offer = await generateLocalPairingOffer({
      polyhiveHome: ctx.polyhiveHome,
      relayEnabled: true,
      relayEndpoint,
  ...
  it("runs `polyhive --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    if (!ctx) throw new Error("test context not initialized");

    const direct = await ctx.polyhive(["ls", "--json"]);
    ...
    const relay = await ctx.polyhive(["ls", "--json", "--host", offerUrl], {
      timeout: 30_000,
      env: { POLYHIVE_HOST: offerUrl },
    });
    ...
    expect([...relayAgents].sort(sortById)).toEqual([...directAgents].sort(sortById));
  }, 60_000);
});
