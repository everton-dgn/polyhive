...
export function isPolyHiveToolName(name: string): boolean {
  const normalized = normalizeToolName(name);
  ...
  if (normalized.includes("__")) {
    ...
    return (
      segments.length >= 3 &&
      segments[0] === "mcp" &&
      (segments[1] === "polyhive" || segments[1]!.startsWith("polyhive_"))
    );
  }
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    return firstSegment === "polyhive" || firstSegment.startsWith("polyhive_");
  }
  return false;
}

export function getPolyHiveToolLeafName(name: string): string | null {
  const normalized = normalizeToolName(name);
  if (normalized.includes("__")) {
    const segments = normalized.split("__").filter((s) => s.length > 0);
    if (
  ...
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    if (firstSegment === "polyhive" || firstSegment.startsWith("polyhive_")) {
