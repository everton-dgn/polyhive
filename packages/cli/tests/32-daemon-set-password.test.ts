...
import { isBearerTokenValid } from "@evertondgn/polyhive-server";
import {
...
const root = await mkdtemp(join(tmpdir(), "polyhive-set-password-"));
const polyhiveHome = join(root, ".polyhive");
...
try {
  {
      ...
      `${JSON.stringify(
        {
          ...
          app: { baseUrl: "https://app.polyhive.sh" },
        },
        null,
        2,
      )}\n`,
    ...
    const result = await setDaemonPasswordInConfig("shared-secret", { home: polyhiveHome });
    const config = JSON.parse(await readFile(join(polyhiveHome, "config.json"), "utf-8"));

    assert.strictEqual(result.configPath, join(polyhiveHome, "config.json"));
    assert.strictEqual(result.restartCommand, "polyhive daemon restart");
