...
export class VoiceAssistantWebSocketServer {
  ...
  public async close(): Promise<void> {
    ...
    // Hard upper bound for any single socket to close during shutdown.
    // If the peer never sends a close frame back, we force-terminate so the
    // overall daemon.stop() stays deterministic even on slow CI runners.
    const SOCKET_CLOSE_TIMEOUT_MS = 2000;
    const closeSocketWithTimeout = (ws: WebSocketLike): Promise<void> =>
      new Promise<void>((resolve) => {
        ...
        const terminate = (ws as WebSocketLike & { terminate?: () => void }).terminate;
    ...
    for (const connection of uniqueConnections) {
      ...
      for (const ws of connection.sockets) {
        cleanupPromises.push(closeSocketWithTimeout(ws));
      }
    }
