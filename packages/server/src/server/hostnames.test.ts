...
describe("hostnames (vite-style)", () => {
  it("allows localhost by default", () => {
    expect(isHostnameAllowed("localhost:6768", undefined)).toBe(true);
  });

  it("allows subdomains of .localhost by default", () => {
    expect(isHostnameAllowed("foo.localhost:6768", undefined)).toBe(true);
  });

  it("allows IP addresses by default", () => {
    expect(isHostnameAllowed("127.0.0.1:6768", undefined)).toBe(true);
    expect(isHostnameAllowed("[::1]:6768", undefined)).toBe(true);
  });

  it("rejects non-default hosts when no allowlist is provided", () => {
    expect(isHostnameAllowed("evil.com:6768", undefined)).toBe(false);
  });

  it("allows any host when set to true", () => {
    expect(isHostnameAllowed("evil.com:6768", true)).toBe(true);
  });

  it("supports leading-dot patterns", () => {
    const hostnames = [".example.com"];
    expect(isHostnameAllowed("example.com:6768", hostnames)).toBe(true);
    expect(isHostnameAllowed("foo.example.com:6768", hostnames)).toBe(true);
    expect(isHostnameAllowed("foo.bar.example.com:6768", hostnames)).toBe(true);
    expect(isHostnameAllowed("notexample.com:6768", hostnames)).toBe(false);
  });
