...
import { THINKING_TONE_PCM_BASE64, THINKING_TONE_PCM_DURATION_MS } from "@/utils/thinking-tone-pcm";

const PCM_MIME_TYPE = "audio/pcm;rate=16000;bits=16";
const KEEP_AWAKE_TAG = "polyhive:voice";
...
export function createVoiceRuntime(deps: VoiceRuntimeDeps): VoiceRuntime {
  ...
  const cuePcm16 = Uint8Array.from(Buffer.from(THINKING_TONE_PCM_BASE64, "base64"));
  const cueSource = {
      ...
      void deps.engine
        ...
        .finally(() => {
          ...
          cue.timeout = setTimeout(
            playNext,
            THINKING_TONE_PCM_DURATION_MS + THINKING_TONE_REPEAT_GAP_MS,
          );
        });
    };
