...
try {
...

  {
    ...
    const posted = await ctx.polyhive(
      ["chat", "post", "coord-room", "first message for @agent-1"],
      {
        env: { POLYHIVE_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
      },
    );
    ...
    const read = await ctx.polyhive(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.polyhive(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    const waitPromise = ctx.polyhive(["chat", "wait", "coord-room", "--timeout", "5s"]);
    await new Promise((resolve) => setTimeout(resolve, 250));
    const secondPost = await ctx.polyhive(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.polyhive(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.polyhive(["chat", "delete", "coord-room"]);
