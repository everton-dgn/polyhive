...
async function getPairingOfferUrl(args: {
  ...
  const pairing = await generateLocalPairingOffer({
    polyhiveHome: args.polyhiveHome,
...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    process.env.POLYHIVE_PRIMARY_LAN_IP = "192.168.1.12";
    ...
    try {
      const offerUrl = await getPairingOfferUrl({
        polyhiveHome: daemon.polyhiveHome,
      ...
      expect(offerUrl.startsWith("https://app.polyhive.sh/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.polyhive.sh:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "polyhive-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestPolyHiveDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      polyhiveHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      const offerUrl1 = await getPairingOfferUrl({
        polyhiveHome: daemon1.polyhiveHome,
      ...
      const daemon2 = await createTestPolyHiveDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        polyhiveHomeRoot: tempHomeRoot,
      ...
      try {
        const offerUrl2 = await getPairingOfferUrl({
          polyhiveHome: daemon2.polyhiveHome,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "polyhive-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      POLYHIVE_HOME: tempHome,
      POLYHIVE_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
      POLYHIVE_DICTATION_ENABLED: "0",
      POLYHIVE_VOICE_MODE_ENABLED: "0",
      POLYHIVE_LOG_FORMAT: "json",
    };
