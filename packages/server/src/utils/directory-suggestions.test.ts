...
describe("searchHomeDirectories", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "polyhive"), { recursive: true });
  ...
  it("returns only existing directories", async () => {
    ...
    expect(results).toContain(path.join(homeDir, "projects", "polyhive"));
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(results).toEqual([path.join(homeDir, "projects", "polyhive")]);
  });
...
describe("searchWorkspaceEntries", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# polyhive\n");
    writeFileSync(
