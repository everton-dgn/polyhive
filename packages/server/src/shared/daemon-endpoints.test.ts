...
describe("relay websocket URL versioning", () => {
  test("defaults relay URLs to v2", () => {
    const url = new URL(
      buildRelayWebSocketUrl({
        endpoint: "relay.polyhive.sh:443",
        serverId: "srv_test",
        role: "client",
      }),
    );
...
describe("relay websocket URLs", () => {
  ...
  test("uses wss for non-443 ports when TLS is enabled", () => {
    const url = new URL(
      buildRelayWebSocketUrl({
        endpoint: "relay.polyhive.sh:6767",
        useTls: true,
        serverId: "srv_test",
        role: "client",
      }),
...
describe("shouldUseTlsForDefaultHostedRelay", () => {
  test("returns true for the hosted PolyHive relay on port 443", () => {
    expect(shouldUseTlsForDefaultHostedRelay("relay.polyhive.sh:443")).toBe(true);
  });
