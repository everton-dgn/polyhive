...
import { resolveLocalPolyHiveHome } from "./local-daemon.js";
...
export async function setDaemonPasswordInConfig(
  ...
  const configPath = path.join(polyhiveHome, CONFIG_FILENAME);
  ...
  savePersistedConfig(polyhiveHome, nextConfig);

  return {
    action: "password_set",
    configPath,
    restartCommand: "polyhive daemon restart",
    message: `Password written to ${configPath}\nRestart the daemon for the change to take effect.\nRun: polyhive daemon restart`,
  };
}
