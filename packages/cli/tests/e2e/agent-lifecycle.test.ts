 ...
 * 2. Create agent with `polyhive run "say hello" --provider claude`
 * 3. List agents with `polyhive ls`
 * 4. Wait for agent with `polyhive wait <id>`
 * 5. Inspect agent with `polyhive inspect <id>`
 * 6. Stop agent with `polyhive stop <id>` and verify it remains inspectable
 * 7. Delete agent with `polyhive delete <id>`
 * 8. Cleanup: stop daemon, remove temp dirs
 *
 * CRITICAL RULES:
 * - NEVER use port 6768 (user's running daemon)
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.polyhive(["ls", "--json"]);
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.polyhive(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.polyhive(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.polyhive(["stop", agentId]);
