...
- **App:** Expo client that runs in browsers (desktop and mobile) and is wrapped by the Electron desktop app.
...
Expo/React Native web app (browser-targeted) that connects to one or more daemons.
...
- Self-hosted relays opt into TLS with `daemon.relay.useTls` or `POLYHIVE_RELAY_USE_TLS=true`
...
- Same WebSocket client as the web app (Electron loads the web bundle)
