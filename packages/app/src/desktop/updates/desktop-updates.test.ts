...
async function loadModuleForPlatform(platform: "web") {
  vi.resetModules();
...
describe("desktop-updates helpers", () => {
  ...
  it("parses local daemon version error result", async () => {
    ...
    expect(
      parseLocalDaemonVersionResult({ version: null, error: "polyhive command not found in PATH" }),
    ).toEqual({
      version: null,
      error: "polyhive command not found in PATH",
    });
  });
