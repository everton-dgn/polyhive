...
const CODEX_IMAGE_ATTACHMENT_DIR = "polyhive-attachments";
...
export class CodexAppServerAgentClient implements AgentClient {
  ...
  async createSession(
    ...
    const session = new CodexAppServerAgentSession(
      ...
      null,
      this.logger,
      (cwd) => this.spawnAppServer(launchContext?.env, cwd ?? sessionConfig.cwd ?? process.cwd()),
      this.deps,
    );
  ...
  async resumeSession(
    ...
    const session = new CodexAppServerAgentSession(
      ...
      handle,
      this.logger,
      (cwd) => this.spawnAppServer(launchContext?.env, cwd ?? merged.cwd ?? process.cwd()),
      this.deps,
    );
    await session.connect();
    return session;
