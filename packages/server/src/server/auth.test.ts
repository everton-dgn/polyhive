...
describe("daemon bearer validator", () => {
  ...
  test("extracts WebSocket polyhive bearer subprotocol tokens", () => {
    const protocol = extractWsBearerProtocol("chat, polyhive.bearer.secret.with.dots");

    expect(protocol).toBe("polyhive.bearer.secret.with.dots");
    expect(extractWsBearerToken(protocol)).toBe("secret.with.dots");
    expect(extractWsBearerToken("polyhive.other.secret")).toBeNull();
  });
});
