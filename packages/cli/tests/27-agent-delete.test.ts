...
const polyhiveHome = await mkdtemp(join(tmpdir(), "polyhive-delete-test-home-"));
...
async function runDelete(args: string[]) {
  return $`POLYHIVE_HOST=localhost:${port} POLYHIVE_HOME=${polyhiveHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
  ...
  {
    console.log("Test 7: polyhive --help shows delete command");
    ...
    console.log("✓ polyhive --help shows delete command\n");
  }

  {
    console.log("Test 8: -q (quiet) flag is accepted with delete");
    const result =
      await $`POLYHIVE_HOST=localhost:${port} POLYHIVE_HOME=${polyhiveHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
