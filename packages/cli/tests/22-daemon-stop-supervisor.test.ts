...
/**
 * Regression: `polyhive daemon stop` must stop supervised dev daemons
...
async function readPidLockState(polyhiveHome: string): Promise<PidLockState> {
  const pidPath = join(polyhiveHome, "polyhive.pid");
...
const polyhiveHome = await mkdtemp(join(tmpdir(), "polyhive-stop-supervisor-"));
...
try {
  ...
  supervisorProcess = spawn(
    ...
    ["--import", "tsx", "../server/scripts/supervisor-entrypoint.ts", "--dev"],
    {
      ...
      env: {
        ...
        POLYHIVE_LISTEN: `127.0.0.1:${port}`,
  ...
  const statusBeforeStop = await readDaemonStatus(polyhiveHome);
  ...
  const pidLockBeforeStop = await readPidLockState(polyhiveHome);
  ...
  console.log("Test 2: `polyhive daemon stop` should stop without respawn");
  const stopResult =
  ...
  const pidAfterStop = await readPidLockState(polyhiveHome);
  ...
  const statusAfterStop = await readDaemonStatus(polyhiveHome);
  assert.strictEqual(
