...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'polyhive-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'PolyHive Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
...
function createTempRepoName(): string {
  ...
  return `polyhive-checkout-ship-${Date.now()}-${rand}`;
}
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createLegacyWorktreeForTest({
          ...
          polyhiveHome: ctx.daemon.polyhiveHome,
        });
        ...
        const worktreeList = await ctx.client.getPolyHiveWorktreeList({
          cwd: repoDir,
        });
        ...
        const archiveResult = await ctx.client.archivePolyHiveWorktree({
          worktreePath: worktree.worktreePath,
        });
        expect(archiveResult.error).toBeNull();
        expect(archiveResult.success).toBe(true);

        const worktreeListAfter = await ctx.client.getPolyHiveWorktreeList({
          cwd: repoDir,
        });
        expect(
