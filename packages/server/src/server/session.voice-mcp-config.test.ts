...
describe("voice MCP stdio config", () => {
  test("builds stdio MCP config for voice agent", () => {
    const config = buildVoiceAgentMcpServerConfig({
      ...
      socketPath: "/tmp/polyhive-voice.sock",
      env: {
        ELECTRON_RUN_AS_NODE: "1",
        POLYHIVE_HOME: "/tmp/polyhive-home",
      },
    });
...
describe("voice mode prompt instructions", () => {
  test("builds enabled voice instructions and preserves base prompt", () => {
    ...
    expect(prompt).toContain("<polyhive_voice_mode>");
    expect(prompt).toContain("PolyHive voice mode is now on.");
  ...
  test("builds disabled voice instructions and supersedes previous voice block", () => {
    ...
    expect(prompt).toContain("Base system prompt");
    expect(prompt).toContain("PolyHive voice mode is now off.");
    expect(prompt).toContain("Ignore any earlier PolyHive voice mode instructions in this thread.");
    expect(prompt.match(/<polyhive_voice_mode>/g)?.length ?? 0).toBe(1);
  ...
  test("strips voice blocks from persisted prompt", () => {
    ...
    expect(
      stripVoiceModeSystemPrompt(
        ["<polyhive_voice_mode>", "legacy voice instruction", "</polyhive_voice_mode>"].join(
          "\n\n",
        ),
      ),
    ).toBeUndefined();
