...
describe("deriveProjectDisplayName", () => {
  it("shows owner/repo for GitHub remote keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "remote:github.com/polyhive/polyhive",
        projectName: "polyhive",
      }),
    ).toBe("polyhive/polyhive");
  });
  ...
  it("falls back to projectName for local keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "/Users/me/dev/polyhive",
        projectName: "polyhive",
      }),
    ).toBe("polyhive");
  });
...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/polyhive" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/polyhive-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:polyhive/polyhive.git",
    });
